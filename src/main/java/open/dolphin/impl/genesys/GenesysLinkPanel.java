...
public class GenesysLinkPanel extends JSplitPane {
    ...
    private String createSeriesText(String[] text) {
        ...
        sb.append("<html>");
        if (text[0].length() > 0) {
            sb.append("<nobr>No.").append(text[0]).append("</nobr><br>");
        }
        if (text[1].length() > 0 && text[2].length() > 0) {
            sb.append("<nobr>").append(text[1]).append(" / ").append(text[2]).append("</nobr><br>");
        } else if (text[1].length() > 0) {
            sb.append("<nobr>").append(text[1]).append("</nobr><br>");
        } else if (text[2].length() > 0) {
            sb.append("<nobr>").append(text[1]).append("</nobr><br>");
        }
        if (text[3].length() > 0) {
            sb.append("<nobr>").append(text[3]).append("</nobr><br>");
        }
        if (text[4].length() > 0) {
            sb.append("<nobr>").append(text[4]).append("</nobr><br>");
        }
        if (text[5].length() > 0) {
            sb.append("<nobr>").append(text[5]).append(" 画像</nobr><br>");
        }
        sb.append("</html>");
        return sb.toString();
    }
