...
//masuda$
    ...
    /**
     * canvasView, toolView の表示位置を計算して設定する 設定前の値として preferences の値を使うバージョン
     *
     * @param srcImage
     * @return
     */
    ...
    /**
     * canvasView, toolView の表示位置を計算して設定する 設定前の値として現在の bounds を使うバージョン
     *
     * @param cv
     * @param tv
     * @param baseImage
    ...
    /**
     * canvasView, toolView の表示位置を計算する canvasView は pack()
     * とかしても自動でいい大きさになってくれないので
     *
    ...
    public void computeViewBounds(SchemaCanvasView cv, SchemaToolView tv, BufferedImage baseImage, Rectangle pCvRect, Rectangle pTvRect) {
        ...
        if (pCvRect == null) {
            ...
            if (cvRect.x < 0) {
                cvRect.x = 0;
            } else if (cvRect.x > screenSize.width - cvRect.width - 100) {
                cvRect.x = screenSize.width - cvRect.width - 100;
            }
            if (cvRect.y < 24) {
                cvRect.y = 24;
            } else if (cvRect.y > screenSize.height - cvRect.height) {
                cvRect.y = screenSize.height - cvRect.height;
            }
        }
        cv.setBounds(cvRect);
