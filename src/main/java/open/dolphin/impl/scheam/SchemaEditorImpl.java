...
/**
 * SchemaEditorImpl ここではコンポネントを作る SchemaEditorProperties: 描画のための変数を扱う StateMgr:
 * ボタンで state を切り替える State: 実際の描画をする部分
 *
 * @author kazm, pns
...
public class SchemaEditorImpl implements SchemaEditor {
    ...
    /**
     * KartePane に SchemaModel を返す 「カルテに展開」ボタン：　createImage で作った BufferedImage
     * を持ってくる 「破棄」ボタン　：　null を持ってくる
     *
     * @param image
     */
...
//s.oh$
        ...
        // その他のボタン
        undoBtn = toolView.getUndoBtn();
        undoBtn.setEnabled(false);
        redoBtn = toolView.getRedoBtn();
        redoBtn.setEnabled(false);
        clearBtn = toolView.getClearBtn();
        clearBtn.setEnabled(editable);
        rotateRightBtn = toolView.getRotateRightBtn();
        rotateLeftBtn = toolView.getRotateLeftBtn();
        expandBtn = toolView.getExpandBtn();
        cancelBtn = canvasView.getCancelBtn();
        okBtn = canvasView.getOkBtn();
    ...
    private void setLineWidthGUI() {
        ...
        if (lw == 1.5f) {
            lineWidthBtn[0].doClick();
        } else if (lw == 2.5f) {
            lineWidthBtn[1].doClick();
        } else if (lw == 3.5f) {
            lineWidthBtn[2].doClick();
        } else if (lw == 4.5f) {
            lineWidthBtn[3].doClick();
        }
