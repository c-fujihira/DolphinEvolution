...
public class UndoMgr {
    ...
    /**
     * recover queue から model をとりだして復活させる present model は store queue に入れる
     *
     * @param recover
     * @param store
     */
    ...
    private void restoreDraw(Model model) {
        ...
        list.clear();
        for (DrawingHolder h : model.getDrawingList()) {
            list.add(h);
        }
    }
    ...
    /**
     * undo 情報を入れておくための model drawingList だけは必ず取っておく。その他は必要に応じてセット
     */
    private class Model {
        ...
        public Model(int code) {
            ...
            for (DrawingHolder h : context.getDrawingList()) {
                drawingList.add(h);
            }
        }
