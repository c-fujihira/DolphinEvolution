...
//s.oh^ 2013/01/29 過去カルテの修正操作(選択状態)
    ...
    public static final Color SELECTED_FGCOLOR = new Color(40, 40, 40);
//s.oh$
    ...
    /**
     * マウスクリック(選択)されたKarteViwerをselectedKarteに設定する。 他のカルテが選択されている場合はそれを解除する。
...
//s.oh$
    ...
    /**
     * 文書履歴の抽出期間が変更された場合、 karteList
     * をclear、選択されているkarteViewerを解除、sateMgrをNoKarte状態に設定する。
     */
    @Override
...
//s.oh$
        ...
        scrollerPanel = new JPanel();
        scrollerPanel.setBackground(new Color(195, 195, 195));
...
//minagawa$         
    ...
    public void modifyKarte() {
        ...
        editor.setMode(mode);
        editor.setKarteDocumentViewer(this);
...
//s.oh$
    ...
    /**
     * 表示選択されているカルテを論理削除する。 患者を間違えた場合等に履歴に表示されないようにするため。
     */
    public void delete() {
...
//minagawa^ 予定カルテ            (予定カルテ対応)
            ...
            if (!newOk && list != null && !list.isEmpty()) {
                boolean allTmp = true;
...
//s.oh$
            ...
            getContext().enabledAction(GUIConst.ACTION_CHECK_INTERACTION, true);   // 併用禁忌チェック
