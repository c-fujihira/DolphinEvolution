 ...
 *
 * modified minagawa オリジナル Git Hub Masuda-Naika OpenDolphin-2.3.8m
...
//minagawa$
    ...
    int retryLimit = Project.getInt("subscribe.retry.count", 100);
    //int retryTime = Project.getInt("subscribe.retry.time", 100);
    int retryTime = Project.getInt("subscribe.retry.time", 1000);
    ...
    private void init() {
        clientUUID = Evolution.getInstance().getClientUUID();
...
//s.oh^ 2013/08/01
                    if (Project.getBoolean("subscribe.retry.check", true)) {
                        ...
                        if (retryCnt > retryLimit) {
                            ...
                            Log.outputFuncLog(Log.LOG_LEVEL_3, Log.FUNCTIONLOG_KIND_WARNING, "同期通信に異常が発生しました。");
                        } else {
                            try {
        ...
        public void run() {
            ...
            while (isRunning) {
                try {
                    ...
                    thread.setPriority(Thread.NORM_PRIORITY);
                    thread.start();
                    System.out.println("同期通信に異常が発生したためsubscribeをリスタートします");
                    Log.outputFuncLog(Log.LOG_LEVEL_3, Log.FUNCTIONLOG_KIND_WARNING, "同期通信に異常が発生したためsubscribeをリスタートします");
                    try {
                        Thread.sleep(retryTime);
                    } catch (InterruptedException ex) {
                    }
    ...
    private void decodeHealthInsurance(PatientModel patient) {
        ...
        if (c != null && !c.isEmpty()) {
            ...
            for (HealthInsuranceModel hm : c) {
                try {
                    // byte[] を XMLDecord
                    PVTHealthInsuranceModel hModel = (PVTHealthInsuranceModel) BeanUtils.xmlDecode(hm.getBeanBytes());
                    list.add(hModel);
                } catch (Exception e) {
                    e.printStackTrace(System.err);
                }
