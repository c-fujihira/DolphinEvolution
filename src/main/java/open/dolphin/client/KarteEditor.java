        ...
        try {
            ...
            if (params != null) {
                ...
                if (getMode() == SINGLE_MODE) {
                    ...
                    boolean cancelFlg = checkInteraction(params);
                    if(cancelFlg){
                        setCancelFlag(true);
                        return;
                    }
                    
                    ChartImpl chartImpl = (ChartImpl)getContext();
                    List<UnsavedDocument> localDirtyList = chartImpl.evoDirtyList();
                    for (UnsavedDocument doc : localDirtyList) {
                        // 保存がcheckされているもののみ追加
                        if (!(doc.getDoc() instanceof KarteEditor)) {
...
//        if (Project.getBoolean(Project.INTERACTION_CHECK) && Project.canSearchMaster()) {
        if (Project.getBoolean(Project.INTERACTION_CHECK)) {
...
//s.oh$
    ...
    public void modifyKarte() {
        karteDocumentViewer.modifyKarte();
    }
    ...
    protected final class NoDirtyState extends EditorState {
        ...
        public void controlMenu() {
            ...
            if (Project.getBoolean(Project.KARTE_SHOW_CONFIRM_AT_NEW, false)) {
    ...
    protected final class SavedState extends EditorState {
        ...
        public void controlMenu() {
            Chart chart = getContext();
            boolean temp = getModel().getDocInfoModel().getStatus().equals(IInfoModel.STATUS_TMP) ? true : false;
            chart.enabledAction(GUIConst.ACTION_NEW_KARTE, !temp);
            chart.enabledAction(GUIConst.ACTION_SAVE, false);
            chart.enabledAction(GUIConst.ACTION_PRINT, true);
            chart.enabledAction(GUIConst.ACTION_MODIFY_KARTE, true);
            chart.enabledAction(GUIConst.ACTION_CUT, false);
            chart.enabledAction(GUIConst.ACTION_COPY, false);
            chart.enabledAction(GUIConst.ACTION_PASTE, false);
            chart.enabledAction(GUIConst.ACTION_UNDO, false);
